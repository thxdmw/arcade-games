...
`三国战纪_v117_正版` 游戏页额外提供“快速集气”开关，默认关闭且刷新页面后仍回到关闭状态。开启后只让玩家 1 的集气槽快速回满，不修改 ROM，也不会附带无限生命、无敌、人物或关卡等其它改版功能；关闭后立即恢复正常集气。

三国战纪 2007 快速集气版的游戏页提供人物出招表，可以按人物姓名、别名或招式名称搜索。PC 端把操作、存档状态合并为紧凑提示，右侧默认至少能完整浏览一名人物的招式；工具栏左边缘可以拖动调整宽度，拖宽后出招卡片自动切换为双列。游戏保持原比例，先减少左右黑色留白，空间不足后才等比缩小。移动端使用全屏弹窗。出招表沿用街机 A/B/C/D 记法，页面会同步显示本站 `J/K/U/I` 的键位对应关系和斜方向组合说明。其它游戏目前不会显示该入口；以后可参照 `assets/move-lists.js` 中的 `kovplus2007` 数据为具体游戏 ID 单独补充，避免不同版本互相套用。
