...
`三国战纪_v117_正版` 提供可选的玩家 1 快速集气功能，默认关闭。进入模拟器底部设置，在 `Backend Core Options` 中把 `Fast Charge PL1` 切换为 `Enabled`；开启后普通攻击命中约两次会获得一管气，受击集气和放招扣气仍按原版规则运行，也不会附带无限生命、无敌、人物或关卡等其它修改。页面不再提供重复开关，关闭同一核心选项即可恢复原版速度；EmulatorJS 通用“秘籍”面板不会列出 FBNeo 原生秘籍。
