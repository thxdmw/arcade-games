...
默认键盘操作为 `WASD` 移动、`J` 攻击、`K` 跳跃、`L` 选道具、`;` 用道具，数字 `1` 投币、数字 `2` 开始。三国战纪的按键名与位次是错位的（游戏内 A 是跳跃、B 是攻击），出招表里的 `+ A` 一律用 `J` 按。游戏页会在非输入区域统一接管按键，并确保短按至少保持数帧；如果浏览器已经保存过旧键位，在底部“控制设置”中点击“重置”。

三国战纪一代的五个正版资源（v117、Plus v119、风云再起 v104、乱世枭雄 v101、一统中原 v201）都提供可选的玩家 1 快速集气功能，默认关闭。进入模拟器底部设置，在 `Backend Core Options` 中把 `Fast Charge PL1` 切换为 `Enabled`；开启后普通攻击命中约两次会获得一管气，其中风云再起、乱世枭雄和一统中原的固定值与动态伤害集气路径都会采用同一速度，避免角色或连段变化时断续。受击集气和放招扣气仍按原版规则运行，也不会附带无限生命、无敌、人物或关卡等其它修改。页面不再提供重复开关，关闭同一核心选项即可恢复原版速度；EmulatorJS 通用“秘籍”面板不会列出 FBNeo 原生秘籍。三国战纪 2 使用不同的 ARM 程序与气槽规则，不复用这一代补丁。

三国战纪 2007 快速集气版的游戏页提供人物出招表，可以按人物姓名、别名或招式名称搜索。PC 端把操作、存档状态合并为紧凑提示，右侧默认至少能完整浏览一名人物的招式；工具栏左边缘可以拖动调整宽度，拖宽后出招卡片自动切换为双列。游戏保持原比例，先减少左右黑色留白，空间不足后才等比缩小。移动端使用全屏弹窗。出招表沿用街机 A/B/C/D 记法，页面会同步显示 `J` 攻击、`K` 跳跃、`L` 选道具、`;` 用道具的对应关系和斜方向组合说明。其它游戏目前不会显示该入口；以后可参照 `assets/move-lists.js` 中的 `kovplus2007` 数据为具体游戏 ID 单独补充，避免不同版本互相套用。
