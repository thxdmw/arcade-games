...
- EmulatorJS 的通用秘籍接口在当前 FBNeo 核心中是空实现；游戏专属功能必须在核心装载前通过 `EJS_externalFiles` 写入 `/fbneo/cheats/<rom>.ini`，再从 `Backend Core Options` 切换 FBNeo 生成的核心选项。4.2.3 会把原始 `ArrayBuffer` 写成 0 字节文件，`prepare-emulator.mjs` 必须先转成 `Uint8Array`。三国战纪一代的快速集气只修改各 ROM 的 68K“攻击命中”路径，目标集气值为 `0x28`：`kov` 为 `0x1149CF/0x11738F`，`kovplus` 为 `0x11A2FD/0x11CCC9`；`kovsh` 除固定值 `0x16563F/0x165645/0x165F11/0x165F17` 外，还需放开两段命中状态门控 `0x165624/0x165EF0/0x165EF6`；`kovshp` 除固定值 `0x165BE3/0x165BE9/0x165C05/0x165C0B/0x1667B5/0x1667BB/0x1667D7/0x1667DD` 外，还需在 `0x165B98/0x166762` 收口动态伤害集气值；`kovytzy` 除固定值 `0x165AF3/0x165AF9/0x166507/0x16650D` 外，还需在 `0x165AC8/0x1664D6` 收口动态伤害集气值。这些版本的集气阈值都是 `0x50`，因此约两次命中获得一管气；受击入口、气数量和玩家 RAM 均不得写入。每个短名称必须使用独立 INI，页面不得提供第二套开关或状态轮询，统一从模拟器设置切换。三国战纪 2 的集气逻辑位于另一套 ARM 程序，不得套用这些地址。
